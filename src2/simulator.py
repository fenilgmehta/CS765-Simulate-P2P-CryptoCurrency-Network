...
class SimulatorParameters:
    ...
    def __init__(self) -> None:
        ...
        self.z_percent_honest_slow_nodes: float = 40.0
        # Number of individual attacker nodes
        self.number_of_individual_attacker_nodes: int = 1
        self.individual_attacker_node_CPU_power_of_total: float = 10.0
    ...
    def load_from_file(self, config_file_name: str):
        ...
        if self.number_of_individual_attacker_nodes * self.individual_attacker_node_CPU_power_of_total > 100:
            ...
            g_logger.error(f'Making number_of_individual_attacker_nodes = {self.number_of_individual_attacker_nodes}')
            g_logger.info(
        ...
        # z% nodes are slow
        self.number_of_slow_nodes: int = int(
            (self.n_total_nodes - self.number_of_individual_attacker_nodes) * self.z_percent_honest_slow_nodes) // 100
    ...
    def log_parameters(self):
        ...
        print(f' CPU Power of an individual attacker node of the total network CPU power = '
...
class Simulator:
    ...
    def initialize(self) -> None:
        ...
        list_malicious: List[bool] = [
            i < self.simulator_parameters.number_of_individual_attacker_nodes
            for i in range(self.simulator_parameters.n_total_nodes)
        ]
        numpy.random.shuffle(list_malicious)
        list_malicious_idx: List[int] = list()
        for i in range(self.simulator_parameters.n_total_nodes):
            if list_malicious[i]:
                list_malicious_idx.append(i)
        g_logger.debug(f'{list_malicious_idx=}')

        # NOTE: "self.simulator_parameters.number_of_slow_nodes" is set appropriately to work with the below logic
        #       number_of_slow_nodes = (n_total_nodes - number_of_individual_attacker_nodes)
        #                              * z_percent_honest_slow_nodes / 100
        def swap_slow_fast(idx):
            for i in range(len(list_slow_fast)):
                # IF node "i" is (slow OR malicious), THEN continue
                if (not list_slow_fast[i]) or list_malicious[i]:
                    continue
                list_slow_fast[i], list_slow_fast[idx] = False, True
                return

        for i in list_malicious_idx:
            if list_slow_fast[i]:
                continue  # IF attacker is fast, then continue
            swap_slow_fast(i)  # Attacker should always be "Fast" node (Given in Assignment 2 PDF)

        # Point 7 of Assignment 1 PDF: Randomly generate CPU power of the nodes
        sp = self.simulator_parameters
        hash_power_percent: List[float] = list(
            numpy.random.random(sp.n_total_nodes - sp.number_of_individual_attacker_nodes)
        )
        hash_power_percent_factor = (100 - sp.number_of_individual_attacker_nodes
                                     * sp.individual_attacker_node_CPU_power_of_total) / sum(hash_power_percent)
        hash_power_percent = [i * hash_power_percent_factor for i in hash_power_percent]
        for i in list_malicious_idx:
            hash_power_percent.insert(i, sp.individual_attacker_node_CPU_power_of_total)
    ...
    def __create_connected_graph(self) -> None:
        ...
        # TODO: update this method to make use of self.simulator_parameters.zeta_attacker_connected_to_honest_nodes_percent
...
def simulator_visualization(mySimulator: Simulator) -> None:
    try:
        print(f'\nAttacker Node Idx = {[node.node_id for node in mySimulator.nodes_list if node.is_malicious]}')
...
def simulation_analysis(mySimulator: Simulator) -> None:
    ...
    for node in mySimulator.nodes_list:
        ...
        if node.is_malicious:
            print('Node belongs to Attacker')
        ...
        # TODO: the message note and formula used are not matching
        print(f'Ratio (longest_chain_len/total_block_gen) = '
              f'{node_blocks_in_longest_chain / node.block_generation_count}')
        print(f'Ratio (nodes mined by this node / len_longest_chain) = '
              f'{node_blocks_in_longest_chain / len_longest_chain}')
        print('Zeta (ζ) = number of honest nodes connected to this attacker = ' + str(
            {len(node.neighbors) / (mySimulator.simulator_parameters.n_total_nodes -
                                    mySimulator.simulator_parameters.number_of_individual_attacker_nodes)}
        ))
        print(f'MPU_node_adv = {node_blocks_in_longest_chain / node.block_generation_count}')
    len_longest_chain_median: int = numpy.median([
        (node.blocks_all[node.blockchain_leafs[-1]].index + 1) for node in mySimulator.nodes_list
    ])
    total_blocks_generated: int = sum([node.block_generation_count for node in mySimulator.nodes_list])
    print(f'MPU_node_overall = {len_longest_chain_median / total_blocks_generated}')
